...
{
    public abstract class FancyScrollRectWithSpecial<TItemData, TContext> : 
        FancyScrollViewWithSpecial<TItemData, TContext>
        where TContext : class, IFancyScrollRectContext, new()
    {
        ...
        protected void RefreshScroller()
        {
            ...
            Scroller.dragSensitivity = ToScrollerPosition(ViewportLength - PaddingHeadLength);
            Scroller.Position = ToScrollerPosition(currentPosition);

            if (Scroller.fancyScrollbar)
            {
                Scroller.fancyScrollbar.gameObject.SetActive(Scrollable);
        ...
        protected virtual void UpdateScrollbarSize(float viewportLength)
        {
            var contentLength = Mathf.Max(ItemsSource.Count + (paddingHead + paddingTail - spacing) / (CellSize + spacing), 1);
            Scroller.fancyScrollbar.size = Scrollable ? Mathf.Clamp01(viewportLength / contentLength) : 1f;
        }
    ...
    public abstract class FancyScrollRectWithSpecial<TItemData> : 
        FancyScrollRectWithSpecial<TItemData, FancyScrollRectContext> { }
