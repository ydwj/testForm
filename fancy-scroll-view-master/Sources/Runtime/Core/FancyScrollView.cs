...
namespace FancyScrollView
{
    ...
    public abstract class FancyScrollView<TItemData, TContext, TFancyCell> : 
        MonoBehaviour where TContext : class, new() where TFancyCell : FancyCell<TItemData,TContext>
    {
        /// <summary>
        /// セル同士の間隔.
        /// </summary>
        ...
        {
            get => loop;
            set => Scroller.SetIsLoop(loop = value);
        }
        ...
        protected readonly IList<TFancyCell> pool = new List<TFancyCell>();
        ...
        protected Scroller cachedScroller;

        /// <summary>
        /// スクロール位置を制御する <see cref="FancyScrollView.Scroller"/> のインスタンス.
        /// </summary>
        protected Scroller Scroller => cachedScroller ? cachedScroller : cachedScroller = GetComponent<Scroller>();
        ...
        protected virtual void UpdateScrollbarSize(float size)
        {
            Scroller.fancyScrollbar.size = size;
        }
        ...
        protected virtual void ResizePool(float firstPosition)
        {
            ...
            {
                var cell = Instantiate(CellPrefab, cellContainer).GetComponent<TFancyCell>();
                if (cell == null)
                {
    ...
    public abstract class FancyScrollView<TItemData> : FancyScrollView<TItemData, NullContext,FancyCell<TItemData>> { }
    
    public abstract class FancyScrollView<TItemData,TContent> : 
        FancyScrollView<TItemData,TContent,FancyCell<TItemData,TContent>> where TContent : class,new() { }
